...
def get_kpop_rankings(by: str, date_from, date_until) -> pd.DataFrame:
    ...
    for year in range(date_from.year, date_until.year + 1):
        ...
        if by == "monthly":
            ...
            period = "week"
            time_max = date_until.isocalendar().week if year == date_until.year else (date.fromisoformat(f'{year}-W01-1') - timedelta(weeks=1)).isocalendar().week
            time_min = date_from.isocalendar().week if year == date_from.year else 1
